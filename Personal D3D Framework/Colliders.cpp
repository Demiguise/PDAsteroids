...
//Plane Collider
PlanarCollider::PlanarCollider(const ModelData& model, float initMass, Entity* parentEnt)
	: RigidBody(initMass, parentEnt)
{
	typeFlag = ColliderType::Planar;
	rbModel = model;
}
...
//Contact Generation
CollisionData* PlanarCollider::GenerateContacts(RigidBody* contactingBody)
{
	CollisionData* data = new CollisionData();
	std::vector<Contact*> contacts;
	switch (contactingBody->typeFlag)
...
{
	bool PlaneAndPlane(	const PlanarCollider* a,
					...
					std::vector<Contact*>& data)
	{
		if (HyperplaneSeperationTest(a,b))
		{
			...
			for (UINT i = 0 ; i < a->rbModel.vData.size() ; ++i)
			{
